...
public interface ExecDetailMapper {
    @Insert("INSERT IGNORE INTO ecs_exec_detail (`key`, `desc`, cron_expr, task_handler_key, init_time, end_time, last_exec_time, " +
            "exec_count, state) " +
    ...
    @Update("UPDATE ecs_exec_detail " +
            "SET `desc` = #{desc}, cron_expr = #{cronExpr}, task_handler_key = #{taskHandlerKey}, init_time = #{initTime}, " +
            "end_time = #{endTime}, last_exec_time = #{lastExecTime}, exec_count = #{execCount}, state = #{state} " +
            "WHERE `key` = #{key}")
    void updateExecDetail(ExecDetail execDetail);

    @Select("SELECT `key`, `desc`, cron_expr, task_handler_key, init_time, end_time, last_exec_time, exec_count, state " +
